...
export const addFiles = action({
  ...
  handler: async (ctx, args) => {
    ...
    const {entryId, created} = await rag.add(ctx,{
      ...
      //If not addded, it will be considered global, we do not want this
      namespace : orgId,
      text,
      key: filename,
      title: filename,
      ...
      contentHash : await contentHashFromArrayBuffer(bytes) // To avoid re-inserting if the file have not changed
    })

    if(!created){
      console.debug("Content already exists, sipping upload metadata");
...
const convertEntryToPublicFile = async(
  ...
  if(storageId){
    try{
      ...
      console.error("Failed to get storage meta data: ", error)
    }
  }
